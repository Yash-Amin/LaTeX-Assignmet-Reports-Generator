...
class OutputImageSection extends Component {
    ...
    render() {
        const handleInputChange = event => {
            // alert('handleChange ' + this.state.id + ' ' + event.target.name);
            const newState = { ...this.state };
            newState[event.target.name] = event.target.value;
            if (event.target.name == 'op-fileUpload') {
                        ...
                        <Grid container alignContent="center">
                            ...
                            <Grid item style={{ padding: '8px' }}>
                                <input
                                    ...
                                    name="op-fileUpload"
                                    onChange={handleInputChange}
                                    // onChange={()=>{alert(this.state.id)}}
                                    id={'op-file-upload-button' + this.state.id}
                                    type="file"
                                />
                                <label
                                    htmlFor={
                                        'op-file-upload-button' + this.state.id
                                    }
                                >
                                    <Button
                                        variant="outlined"
...
const mapStateToProps = state => {
    return {
        items: state.outputOutputImageSection
    };
};
